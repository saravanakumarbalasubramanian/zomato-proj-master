...
const LargeTabs = () => {
 ...
 <Link to={`/${items.id}`}>
        ...
        <div className={`w-12 h-12  bg-${ type === items.id ? items.activecolor : "grey"}-100  p-2 rounded-full`}>
            <img src={ type === items.id ? items.imageActive : items.imageDefault}
                 alt="Delivery"
                  className='w-full h-full '/>
        </div>
        <h3 className={type === items.id ? "text-xl text-zomato-400" : "text-xl text-grey-700" }> { items.name } </h3>
       </div>
